...
`HWTW.exe`, **`HWTW.msi`**, macOS **`.dmg`**, **`.app.zip`**, **`.pkg`**, `requirements.txt`, **`SHA256SUMS.txt`** (Windows + MSI), and **`SHA256SUMS-macos.txt`** (DMG + zip + pkg, when the macOS job completes) on GitHub **Releases** are produced by the **Release** workflow. Compare the tag to the workflow run and commit SHA when verifying binaries; use `sha256sum -c SHA256SUMS.txt` on Linux or compare hashes manually.
